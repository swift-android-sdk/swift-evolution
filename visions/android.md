...
Packaging Swift code and its runtime into Android application packages (APKs) is necessary in order to create a distributable application. Guidelines and tooling for correctly packaging compiled Swift binaries and necessary runtime libraries into the native `lib/<arch>` folder of an APK will be provided. We are trying out generating Android App Bundles (AABs), allowing efficient delivery of architecture-specific Swift code to end-user devices via the Android app stores.
